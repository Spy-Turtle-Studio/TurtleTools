...
// Edited by Dennis at SpyTurtleStudio
...
namespace spyturtlestudio.tools.Editor.CustomInspector
{
	...
	public class ExtendedScriptableObjectDrawer : PropertyDrawer 
    {
		...
		public static T _GUILayout<T> (string label, T objectReferenceValue, ref bool isExpanded) where T : ScriptableObject => 
			_GUILayout<T>(new GUIContent(label), objectReferenceValue, ref isExpanded);
		...
		public static T _GUILayout<T> (GUIContent label, T objectReferenceValue, ref bool isExpanded) where T : ScriptableObject 
		{
			...
			if(objectReferenceValue != null) {
				...
				var indentedPosition = EditorGUI.IndentedRect(position);
				var indentOffset = indentedPosition.x - position.x;
				new Rect(position.x + EditorGUIUtility.labelWidth - indentOffset, position.y, position.width - EditorGUIUtility.labelWidth - indentOffset, EditorGUIUtility.singleLineHeight);
			} 
			else 
			{
				foldoutRect.x += 12;
				...
				var indentedPosition = EditorGUI.IndentedRect(position);
				var indentOffset = indentedPosition.x - position.x;
				new Rect(position.x + EditorGUIUtility.labelWidth - indentOffset, position.y, position.width - EditorGUIUtility.labelWidth - indentOffset-60, EditorGUIUtility.singleLineHeight);
			}
